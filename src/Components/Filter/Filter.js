...
const Filter = ({ leadsCols, filter, module, filterMethodNames }) => {
    ...
    const hideFilter = () => { if (showFilterMethods) setShowFilterMethod(false) };


    const filterMethodsElements = filterMethodNames.map((method, index) => <li onClick={() => setFilterByMethod(method)} key={index}>{method}</li>);

    return (
        <section className={styles.filterSection} onClick={hideFilter}>
            <p>Filter {module} By:</p>
            { leadsCols.map((col, index) => <div key={index} className="filterby"> 
                <div className={styles.filterOptions}>
                <input type="checkbox" onClick={() => { createFilterCriteria(col.label) }} />
                <p className={styles.lighter}>{col.label}</p>
            </div>
                {
                    showFilter && filterValue === col.label.toLowerCase().split(" ").join("_") && 
                    <> 
                        <div onClick={() => { setShowFilterMethod(prevState => !prevState) }} className={styles.filterCriteria}>
